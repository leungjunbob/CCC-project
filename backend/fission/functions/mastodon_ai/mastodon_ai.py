...
def main() -> str:
    ...
    for post in cleaned_posts :
        try:
            es_client.index(index="mastodon_ai", id=post["id"], body=post)
            success += 1
    ...
    return f" Indexed {success} posts to mastodon_ai"
...
def get_last_id(es_client):
    try:
        res = es_client.get(index="mastodon_ai_last_id", id="last_id")
        return res["_source"]["last_id"]
    except Exception:
        return None  # first fetching



def save_last_id(es_client, last_id):
    es_client.index(
        index="mastodon_ai_last_id",
        id="last_id",
        body={
            "last_id": last_id
