...
def main() -> str:
    ...
    for post in cleaned_posts :
        try:
            es_client.index(index="mastodon_weather", id=post["id"], body=post)
            success += 1
    ...
    return f" Indexed {success} posts to mastodon_weather"
...
def get_last_id(es_client):
    try:
        res = es_client.get(index="mastodon_meta", id="last_id")
